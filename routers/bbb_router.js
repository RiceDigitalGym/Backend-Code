		...
		if (session) {
			...
			utils.findRecentBikeData(session.sessionID).then(function(data) {
				...
				if (data) {
					res.send({
						success:true, 
						message: "Your current rpm is " + parseInt(data.dataValues.rpm)
						rpm: data.dataValues.rpm 
					})
					console.log("here4")
				}
			});
			...
			utils.findRecentBikeData(551).then(function(data){ //testing 
				console.log("here5")
				console.log(data)
				if (data) {
					res.send ({
						success:false,
						message: "Your last rpm was " + parseInt(data.dataValues.rpm),
						rpm: data.dataValues.rpm 
					})
				}
			})
